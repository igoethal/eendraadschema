class SituationPlan {
    ...
    addElementFromFile(event: any, page: number, posx: number, posy: number, callback): SituationPlanElement {
        let element: SituationPlanElement = new SituationPlanElement(page,posx,posy,0,0,0,SITPLANVIEW_DEFAULT_SCALE,randomId("SP_"),"");
    ...
    addElementFromSVG(svg: string, page: number, posx: number, posy: number) {
        let element: SituationPlanElement = new SituationPlanElement(page,posx,posy,0,0,0,SITPLANVIEW_DEFAULT_SCALE,randomId("SP_"),svg);
    ...
    addElectroItem(id: number, page: number, posx: number, posy: number, adrestype: string, adres:string, adreslocation: string,
                   scale: number, rotate: number): SituationPlanElement {
    ...
    fromJsonObject(json: any) {
        ...
        for (let element of json.elements) {
            let newElement = new SituationPlanElement(1,0,0,0,0,0,SITPLANVIEW_DEFAULT_SCALE,randomId("SP_"),"");
    ...
    toSVG() {
        ...
        for (let element of this.elements) {
            ...
            outstr += `<text x="${element.labelposx}" y="${element.labelposy}" font-size="11" fill="black" text-anchor="middle" dominant-baseline="middle">${element.getAdres()}</text>`
        }

        outstr = `<svg xmlns="http://www.w3.org/2000/svg" width="${maxx}px" height="${maxy}px" viewBox="0 0 ${maxx} ${maxy}">${outstr}</svg>`;
        ...
        for (let i=0; i<outstruct.numpages; i++ ) {
            ...
            for (let element of this.elements) {
                if (element.page == (i+1)) {
                    ...
                    maxx = Math.max(maxx, element.posx + element.sizex/2);
                    maxy = Math.max(maxy, element.posy + element.sizey/2);
                    svgstr += `<text x="${element.labelposx}" y="${element.labelposy}" font-size="11" fill="black" text-anchor="middle" dominant-baseline="middle">${element.getAdres()}</text>`
                }
            }
